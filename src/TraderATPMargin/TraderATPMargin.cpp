...
#ifdef _WIN32
#pragma comment(lib, "../API/AtpTradeApiMarginV4.1.1/x64/atptradeapi.lib")
#endif
...
inline WTSBusinessType wrapBusinessType(ATPBusinessTypeType bType)
{
	WTSBusinessType mType;
	switch (bType)
	{
	...
	case ATPBusinessTypeConst::kETFRealTimePurchaseRedemption:
		mType = BT_ETF;
	case ATPBusinessTypeConst::kOptionExercise:
		mType = BT_EXECUTE;
	case ATPBusinessTypeConst::kOptionAuction:
		mType = BT_QUOTE;
	case ATPBusinessTypeConst::kMarginTrade:
		mType = BT_CREDIT;
	default:
		mType = BT_UNKNOWN;
		break;
...
inline ATPBusinessTypeType wrapBusinessType(WTSBusinessType oType)
{
	ATPBusinessTypeType mType;

	switch (oType)
	{
	case BT_CASH:
		mType = ATPBusinessTypeConst::kCashAuction;
	case BT_ETF:
		mType = ATPBusinessTypeConst::kETFRealTimePurchaseRedemption;
	case BT_EXECUTE:
		mType = ATPBusinessTypeConst::kOptionExercise;
	case BT_QUOTE:
		mType = ATPBusinessTypeConst::kOptionAuction;
	case BT_CREDIT:
		mType = ATPBusinessTypeConst::kMarginTrade;
	default:
		mType = ATPBusinessTypeConst::kDefault;
		break;
	}
...
WTSTradeInfo* TraderATP::makeTradeRecord(const ATPRspCashAuctionTradeERMsg *cash_auction_trade_er)
{
	...
	pRet->setBusinessType(wrapBusinessType(cash_auction_trade_er->business_type));
	pRet->setTradeType(WTT_Common);
...
WTSTradeInfo* TraderATP::makeTradeRecord(const ATPRspCreditTradeERMsg &cash_credit_trade_er)
{
	const std::string code(cash_credit_trade_er.security_id);
	const std::string exchg = (cash_credit_trade_er.market_id == ATPMarketIDConst::kShangHai) ? "SSE" : "SZSE";
	...
	WTSTradeInfo *pRet = WTSTradeInfo::create(code.c_str(), exchg.c_str());
	pRet->setVolume((uint32_t)cash_credit_trade_er.last_qty / 100);  // 成交数量
	pRet->setPrice(cash_credit_trade_er.last_px / 10000.0);  // 成交价格
	pRet->setTradeID(cash_credit_trade_er.exec_id);
	pRet->setContractInfo(contract);

	uint32_t uTime = (uint32_t)(cash_credit_trade_er.transact_time % 1000000000);
	uint32_t uDate = (uint32_t)(cash_credit_trade_er.transact_time / 1000000000);

	pRet->setTradeDate(uDate);
	pRet->setTradeTime(TimeUtils::makeTime(uDate, uTime));

	WTSDirectionType dType = wrapATPSide(cash_credit_trade_er.side);
	pRet->setDirection(dType);
	//pRet->setOffsetType(wrapOffsetType(cash_auction_trade_er->side));
	fmtutil::format_to(pRet->getRefOrder(), "{}", cash_credit_trade_er.cl_ord_id);
	pRet->setTradeType(WTT_Common);
	pRet->setBusinessType(wrapBusinessType(cash_credit_trade_er.business_type));

	double amount = cash_credit_trade_er.total_value_traded / 10000.0;   //成交金额
	pRet->setAmount(amount);
...
void TraderATP::OnError(const std::string& reason)
{
	if (_sink)
		write_log(_sink, LL_ERROR, "[TraderATP] {}", reason);
}
...
void TraderATP::OnConnectFailure(const std::string& reason)
{
	...
	write_log(_sink, LL_WARN, "[TraderATP] {}...", reason);
}
...
void TraderATP::OnRspOrderStatusInternalAck(const ATPRspOrderStatusAckMsg& order_status_ack)
{
	if (order_status_ack.reject_reason_code != ATPRejectReasonCodeConst::kNormal)
	{
		...
		// 对于撤单返回的状态响应，为原始委托的客户合同号，指示被撤消订单的cl_ord_no; 
		// 对于委托确认，取值为0
		if (order_status_ack.orig_cl_ord_no != 0)
		{
			// 对应撤单回调
...
// 两融交易成交回报
void TraderATP::OnRspCreditTradeER(const ATPRspCreditTradeERMsg& credit_trade_er)
{
	write_log(_sink, LL_INFO, "[OnRspCreditTradeER] code: {}, price: {}, qty: {}, cum_qty: {}, left_qty: {}, order_status: {}, cl_ord_no: {}, order_id: {}, cl_ord_id: {}, fee: {}, cash_margin: {}",
		credit_trade_er.security_id, credit_trade_er.price, credit_trade_er.order_qty, credit_trade_er.cum_qty, credit_trade_er.leaves_qty, credit_trade_er.ord_status,
		credit_trade_er.cl_ord_no, credit_trade_er.order_id, credit_trade_er.cl_ord_id, credit_trade_er.fee, credit_trade_er.cash_margin);

	WTSTradeInfo *tRecord = makeTradeRecord(credit_trade_er);
	if (tRecord)
	{
		_asyncio.post([this, tRecord] {
			if (_sink)
	...
	// 保存回报分区号、序号，用于断线重连时指定已收到最新回报序号
	report_sync[credit_trade_er.partition] = credit_trade_er.index;
}
...
void TraderATP::OnRspQueryFundAssetsInfoQueryResult(const ATPRspQueryFundAssetsInfoResultMsg& msg)
{
	WTSArray* ayFunds = WTSArray::create();
	WTSAccountInfo* fundInfo = WTSAccountInfo::create();
	fundInfo->setMargin(msg.margin_required);
	fundInfo->setFrozenMargin(msg.margin_pre_required);
	fundInfo->setBalance(msg.fund_balance / 10000.0);
...
void TraderATP::OnRspMarginRepayTradeER(const ATPRspMarginRepayTradeERMsg& margin_trade_repay_er)
{
	if (_sink)
		write_log(_sink, LL_INFO, "OnRspMarginRepayTradeER: ");
}

void TraderATP::OnRspQuoteStatusAck(const ATPRspQuoteStatusAckMsg& order_status_ack)
{
	if (_sink)
		write_log(_sink, LL_INFO, "OnRspQuoteStatusAck: ");
}
...
int TraderATP::orderInsert(WTSEntrust* entrust)
{
	...
	wt_strcpy(p->security_id, entrust->getCode());
	...
	// TODO
	//p->stop_px = (int32_t)(entrust->getPrice() * 10000);         // 止损价格 N13(4)，21.0000元
	...
	//for (int i = 0; i < totals; i++)
	{
		...
		ATPRetCodeType ec = _api->ReqCashAuctionOrder(p);  //担保品交易
		if (ec != ErrorCode::kSuccess)
		{
