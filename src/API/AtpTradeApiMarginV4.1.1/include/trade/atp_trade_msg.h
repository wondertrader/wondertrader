// code_generator_tag
// messages:e733758a9d8f0a6ec25d135dd2ad80c6 template:d5e676b48a16d8942e0c6cea656a6229 code:3ae66a742a45c34004c439036ee62344
...
struct TRADE_API ATPReqMarginNoTradeOrderMsg
{
    ...
    ATPImmediateEffectType immediate_effect; ///实时划转标志,0:非实时划转,1:实时划转
...
struct TRADE_API ATPReqSwapPutbackOrderMsg
{
    ...
    ATPEnforceFlagType enforce_flag; ///风险强制标识（已弃用）
    ATPShareNegotiabilityType share_negotiability; ///股份性质，定向可转债转股申报必填
...
{
    ATPSecurityIDType security_id; ///成分股证券代码，例外：1-沪市资金代码、2-深市或京市资金代码、3-港市资金代码、9-非沪港深京资金代码（全额现金申赎）
...
struct TRADE_API ATPReqOrderQueryMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码（默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有。开通北交所业务后必填）
...
struct TRADE_API ATPReqBatchOrderQueryMsg
{
    ...
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPOrderQueryConditionType order_query_condition; ///委托查询条件 （默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPMarketIDType market_id; ///市场代码，开通北交所业务后必填（ATP V3.4版本开始支持）
...
struct TRADE_API ATPReqTradeOrderQueryMsg
{
    ...
    ATPExecIDType exec_id; ///执行编号
    ATPEtfPurchaseRedemptionFlagType etf_purchase_redemption_flag; ///是否返回ETF申赎成交：0-不返回，1-返回（ATP3.2.1开始支持）
    ATPBatchClOrdNoType batch_cl_ord_no; ///客户自定义委托批号(ATP V3.2.3版本开始支持)
...
struct TRADE_API ATPReqExtQueryContractSpecificationsMsg
{
    ...
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
    ATPReturnSeqType return_seq; ///返回顺序，按时间排序 （默认值为1，表示按时间正序排序）
    ATPContractSerialNumType contract_serial_num; ///合同序号（ATP4.0.0版本开始支持）
...
struct TRADE_API ATPReqShareQueryMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码 （默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有。开通北交所业务后必填）（ATP3.1.6版本开始支持此字段非必填）
    ATPSecurityIDType security_id; ///证券代码 （默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
    ATPReportIndexType query_index; ///起始查询索引号 （默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqExtFundTransferOtherMsg
{
    ...
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPMarketIDType market_id; ///市场代码，开通北交所业务后必填（ATP V3.4版本开始支持）
    ATPFundPropertyType fund_property; ///资金属性：'0'=T+0 '1'=T+1 ，可指定要划拨的资金属性，当不填时以配置的全局资金划拨参数为准。（ATP V4.0.0版本开始支持）
...
struct TRADE_API ATPReqAgwUserPasswdModifyMsg
{
    ...
    ATPFundAccountIDType fund_account_id; ///资金账户（当登录模式为2-资金账户登录模式时，该字段必填）
    ATPBranchIDType branch_id; ///营业部（当登录模式为2-资金账户登录模式时，该字段是否必填请咨询券商）
    ATPPasswordType password; ///客户密码
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPOrderWayType order_way; ///委托方式（是否必填以及字典项请咨询券商）
    ATPLoginModeType login_mode; ///登录模式：1-客户号登录 2-资金账户登录
    ATPAgwUserPasswdType agw_user_new_password; ///网关用户新密码。密码强度要求：密码长度为8-16个字符，至少包含数字,字母和特殊符号~!@#$%^&*?的两项以上
    ATPUserInfoType user_info; ///用户私有信息
    ATPExtraData extra_data; ///扩展字段
...
struct TRADE_API APIContractSpecificationsManagerUnit
{
    ...
    ATPAmtType repayment_other_debt; ///已偿还其他息费N18(4)（融资融券逾期罚息已偿还金额）
...
struct TRADE_API ATPRspQueryFundAssetsInfoResultMsg
{
    ...
    ATPAmtType rr_other_debt; ///其他负债（除融资利息、融券费用、股份权益补偿外的其他利息负债）N18(4)
...
{
    ATPBusinessTypeType business_type; ///业务类型
    ATPSecurityIDType security_id; ///证券代码（对于ETF实时申赎成交，上海市场：1-沪市资金代码、2-深市或京市资金代码、3-港市资金代码、9-非沪港深京资金代码）
    ATPSecuritySymbolType security_symbol; ///证券简称
    ATPMarketIDType market_id; ///市场代码
    ATPAccountIDType account_id; ///证券账户ID
    ...
    ATPBoolType cancel_flag; ///撤单标识：0-非撤单，1-撤单（ATP3.5 开始支持，3.5之前固定为0，勿用）
    ATPRejectReasonCodeType reject_reason_code; ///拒绝原因代码；可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-5 拒绝原因错误码类型，即可查询到错误描述信息（ATP3.5 开始支持，3.5之前固定为0，勿用）
    ATPOrdRejReasonType ord_rej_reason; ///拒绝原因描述信息；当交易所拒绝时，这里填写交易所返回的拒绝原因代码。可调用GetErrorInfo错误码查询函数，传入错误码及错误码类型-1交易所拒绝原因错误码类型，即可查询错误描述信息（ATP3.5 开始支持，3.5之前固定为空，勿用）
...
{
    ATPSecurityIDType security_id; ///证券代码，例外：1-沪市资金代码、2-深市或京市资金代码、3-港市资金代码、9-非沪港深京资金代码（全额现金申赎）
...
struct TRADE_API APIETFTradeOrderUnit
{
    ...
    ATPClOrdIDType cl_ord_id; ///申报合同号,上交所：以QP1开头,表示为交易所保证金强制平仓；以CV1开头,表示为交易所备兑强制平仓；
    ATPTimeStampType transact_time; ///成交时间，对于交易所确认撤单，取交易所确认撤单的回报时间
...
struct TRADE_API ATPReqExternalInsTETransFundReqMsg
{
    ...
    ATPClientFeatureCodeType client_feature_code; ///终端识别码（该字段是否必填请咨询券商）
    ATPUserInfoType user_info; ///用户私有信息
    ATPFundPropertyType fund_property; ///资金属性：'0'=T+0 '1'=T+1 ，可指定要划拨的资金属性，当不填时按原有逻辑T+1处理。（ATP V4.0.0版本开始支持）
...
struct TRADE_API APIOptionOrderUnit
{
    ...
    ATPParticularFlagsType particular_flags; ///特殊委托标识，右起第一个bit位是夜市委托标志位：0-正常委托、1-夜市委托，目前默认为0；右起第二个bit位是协议行权标志位：0-非协议行权、1-协议行权（ATP3.3.0开始支持）
...
struct TRADE_API ATPReqExtQueryETFComponentInfoMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码
    ATPSecurityIDType security_id; ///ETF代码
    ATPSecurityIDType component_security_id; ///成分股代码（默认值为0，表示查所有） 
    ATPReportIndexType query_index; ///起始查询索引号 （默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回，具体数量请咨询券商）
...
struct TRADE_API ATPReqNewShareHitQueryMsg
{
    ...
    ATPMarketIDType market_id; ///市场代码
    ATPSecurityIDType security_id; ///证券代码
    ATPDate8Type start_date; ///申购开始日期（日期格式YYYYMMDD)，默认值为0，表示查所有
...
struct TRADE_API ATPReqExtQueryRepaymentDetailMsg
{
    ...
    ATPReportIndexType query_index; ///起始查询索引号 （默认值为0，表示从头开始查）
    ATPTotalNumType return_num; ///查询返回数量 （默认值为0，表示按能返回的最大数量返回。受单次最大返回参数控制，可配置。）
    ATPSeqNumType client_seq_id; ///用户系统消息序号
    ATPUserInfoType user_info; ///用户私有信息
    ATPPasswordType password; ///客户密码
    ATPExtraData extra_data; ///扩展字段 
...
struct TRADE_API ApiRepaymentDetailArray
{
    ...
    ATPClOrdIDType contract_bail_serial_num; ///委托序号（本笔偿还生成的报盘合同号）
    ATPExecIDType exec_id; ///执行编号
    ATPClOrdNOType cl_ord_no; ///客户订单编号
