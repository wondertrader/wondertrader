// code_generator_tag
// requests:e0926c423290e5d030fa2fff8376026d template:003bbb5d74c451382456bdbedeb4a71a code:57d109709b972005e82f9b68da93692d
...
class TRADE_API ATPTradeAPI
{
    ...
    public:
         ...
         *              ENCRYPT_SCHEMA “0”：表示API进行转发，API不再进行加密。“1”：表示API对消息中的密码字段进行密码
         ...
         *              注：如果ENCRYPT_SCHEMA = 1时，未配置ATP_LOGIN_ENCRYPT_PASSWORD时将采用默认内置RSA算法，未配置ATP_ENCRYPT_PASSWORD时将采用默认内置SHA256算法
         ...
         * @type       order
         * @note       支持沪深市场主板、科创板、创业板、北交所的股票、债券、基金的竞价买卖业务
         */
         ...
         * @business   cash,margin
         * @type       cancel
         * @note       支持以下业务类型的撤单：盘后定价交易、大宗交易、债券分销、现货交易、ETF、盘后定价（科创板、创业板）交易业务、港股通买卖、港股通公司行为、网上发行认购、网络投票、质押式回购、配售业务、债券转股回售、要约收购、权证行权<br/>按“客户自定义委托批号”处理撤单时，若找不到对应原订单、或因其他原因撤单请求被拒绝,ATP不会生成Ack响应消息。
         */
         ...
         * @note       
         */
        ATPRetCodeType ReqExtQueryETFComponentInfo(const ATPReqExtQueryETFComponentInfoMsg* msg);
         ...
         * @business   common
         * @type       other
         * @note       客户号/资金账户及密码用于身份认证，保障安全性。密码修改成功后，系统会断开被修改网关用户当前已创建的连接
         */
        ATPRetCodeType ReqAgwUserPasswdModify(const ATPReqAgwUserPasswdModifyMsg* msg);
         ...
         * @note       
         */
        ATPRetCodeType ReqExtQueryRepaymentDetail(const ATPReqExtQueryRepaymentDetailMsg* msg);
