#if !defined(__MA_CLI_FS2_API_H__)
#define __MA_CLI_FS2_API_H__
...
{
  char          szSvrAddress[32 + 1];       // 服务器IP地址
  int           nSvrPort;                   // 服务器端口
...
struct MATRADEAPI CReqFixField
{
  ...
  int           iOpOrg;                     // 操作机构
  char          szServiceId[32 + 1];        // 服务目录
};

// 终端信息参数
struct MATRADEAPI CReqClientField
{
  char          szIip[15 + 1];              // 公网IP
  char          szIport[5 + 1];             // 公网IP端口号
  char          szLip[15 + 1];              // 内网IP
...
public:
  ...
  ///注册回调接口
  virtual int RegisterSpi(CCliFs2Spi *p_pSpi);
  ...
  // 设置业务包头信息
  int SetBizPackHead(const char *p_pszFuncId, LONGLONG p_llRequestId, char *p_pszMsgId, char p_chPkgType, char p_chPkgVer = '3');
