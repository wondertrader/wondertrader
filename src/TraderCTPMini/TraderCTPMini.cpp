...
int TraderCTPMini::doLogin()
{
	...
	if (iResult != 0)
	{
		m_traderSink->handleTraderLog(LL_ERROR, "[CTPMiniTrader]登录请求发送失败, 错误码:%d", iResult);
	}
...
int TraderCTPMini::logout()
{
	...
	if (iResult != 0)
	{
		m_traderSink->handleTraderLog(LL_ERROR, "[CTPMiniTrader]注销请求发送失败, 错误码:%d", iResult);
	}
...
int TraderCTPMini::orderInsert(WTSEntrust* entrust)
{
	...
	if (iResult != 0)
	{
		m_traderSink->handleTraderLog(LL_ERROR, "[CTPMiniTrader]插入订单失败, 错误码:%d", iResult);
	}
...
int TraderCTPMini::orderAction(WTSEntrustAction* action)
{
	...
	if (iResult != 0)
	{
		m_traderSink->handleTraderLog(LL_ERROR, "[CTPMiniTrader]撤单请求发送失败, 错误码:%d", iResult);
	}
...
{
	//m_traderSink->handleTraderLog(LL_ERROR, "[CTPMiniTrader]CTP交易服务器已断开");
	m_wrapperState = WS_NOTLOGIN;
	if (m_traderSink)
...
void TraderCTPMini::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if (!IsErrorRspInfo(pRspInfo))
	{
		...
		m_traderSink->handleTraderLog(LL_INFO, "[%s-%s]账户登录成功，AppID:%s, Sessionid: %u, 登录时间: %s……",
		...
		if(lastDate != m_lDate)
		{
			...
			m_traderSink->handleTraderLog(LL_INFO, "[%s-%s]交易日已切换[%u -> %u]，清空本地数据缓存……", m_strBroker.c_str(), m_strUser.c_str(), lastDate, m_lDate);
		}

		m_traderSink->handleTraderLog(LL_INFO, "[%s-%s]账户登录成功，交易日：%u……", m_strBroker.c_str(), m_strUser.c_str(), m_lDate);
...
WTSTradeInfo* TraderCTPMini::makeTradeRecord(CThostFtdcTradeField *tradeField)
{
	...
	pRet->setOffsetType(wrapOffse浜ゆ