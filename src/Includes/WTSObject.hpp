...
{
private:
	static boost::thread_specific_ptr<boost::object_pool<T>> _pool;
...
public:
	...
	{
		if (!_pool.get())
			_pool.reset(new boost::object_pool<T>());
...
template<typename T>
boost::thread_specific_ptr<boost::object_pool<T> > WTSPoolObject<T>::_pool;
NS_WTP_END
