...
void TraderATP::OnRspShareQueryResult(const ATPRspShareQueryResultMsg &msg)
{
	...
	for (auto &unit : msg.order_array) {
		...
		if (contract)
		{
			...
			pos->setNewPosition((double)(unit.leaves_qty / 100.0));  // 剩余股份数量N15(2)，包含当日买入部分，拆分合并，申购赎回
			...
			pos->setAvailNewPos(unit.available_qty / 100.0);  // 可用股份数量N15(2)
...
void TraderATP::doLogin(const char* productInfo)
{
	...
	login_msg.login_mode = _loginmode;  // 登录模式
...
int TraderATP::orderInsert(WTSEntrust* entrust)
{
	...
	p.market_id = isSh ? ATPMarketIDConst::kShangHai : ATPMarketIDConst::kShenZhen;             // 市场ID，上海
	wt_strcpy(p.cust_id, _cust_id.c_str());                 // 客户号ID
	wt_strcpy(p.fund_account_id, _fund_accountid.c_str());       // 资金账户ID

	if (isSh)
		wt_strcpy(p.account_id, _sh_acctid.c_str());                 // 账户ID
	else
...
int TraderATP::orderAction(WTSEntrustAction* action)
{
	...
	if (isSH)	
		wt_strcpy(p.account_id, _sh_acctid.c_str());  // 证券账户
	else
		wt_strcpy(p.account_id, _sz_acctid.c_str());  // 证券账户
...
int TraderATP::queryPositions()
{
	...
	_asyncio.post([this]() {
		if (!_sh_acctid.empty())
		{
			...
			if (_sh_total_num > 100)  // 最大单页容量为100条
			{
				// 需要多页查询
