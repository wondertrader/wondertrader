...
void TraderXTPXAlgo::OnTradeEvent(XTPTradeReport *trade_info, uint64_t session_id)
{
	...
	std::cout << " order_xtp_id: " << trade_info->order_client_id;
	std::cout << " order_xtp_id: " << trade_info->ticker;
	std::cout << " order_xtp_id: " << trade_info->market;
...
void TraderXTPXAlgo::OnALGOUserEstablishChannel(char* user, XTPRI* error_info, uint64_t session_id)
{
	...
	if (error_info->error_id == 0)
	{
		...
		int ret = _api->InsertAlgoOrder(_algo_type, _client, (char*)_algo_params.c_str(), session_id);
		if (ret == 0)
		{
		...
		else
		{
			...
			XTPRI* error_info = _api->GetApiLastError();
...
void TraderXTPXAlgo::OnInsertAlgoOrder(XTPStrategyInfoStruct* strategy_info, XTPRI *error_info, uint64_t session_id)
{
	...
	int ret = _api->CancelAlgoOrder(true, strategy_info->m_xtp_strategy_id, session_id);
	if (ret == 0)
	{
...
bool TraderXTPXAlgo::init(WTSVariant *params)
{
	...
	_host_oms = params->getCString("host");
	_port_oms = params->getInt32("port");
	...
	_host_algo = params->getCString("host");
	_port_algo = params->getInt32("port");
...
void TraderXTPXAlgo::doLogin()
{
	...
	iResult = _api->LoginALGO(_host_algo.c_str(), _port_algo, _acct_algo.c_str(), _password_algo.c_str(), XTP_PROTOCOL_TCP);
	...
	else
	{
		...
		if (user_ret == 0) {
			write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Establish channel send success: {}", user_ret);
		}
		else {
			auto error_info = _api->GetApiLastError();
...
int TraderXTPXAlgo::queryOrders()
{
	...
	//int iResult = _api->QueryStrategy(_algo_type, _client, )
