...
void ParserXTP::OnSubMarketData(XTPST *ticker, XTPRI *error_info, bool is_last)
{
	...
	{
		if(m_sink)
			write_log(m_sink, LL_ERROR, "[ParserXTP] Market data subscribe failed, code: {}.{}, msg: {}, err code: {}, err msg: {}", ticker->exchange_id == XTP_EXCHANGE_SH ? "SSE" : "SZSE", ticker->ticker, error_info->error_id, error_info->error_msg);
	}
}
...
void ParserXTP::DoLogin()
{
	...
	//write_log(m_sink, LL_INFO, "host: {}, port: {}, protocal type: {}, user: {}, pass: {}, client: {}, local_ip: {}", m_strHost, m_iPort, m_iProtocol, m_strUser, m_strPass, m_uClientID, m_strLocalIP);

	int iResult = 0;
	if (!m_strLocalIP.empty())
		iResult = m_pUserAPI->Login(m_strHost.c_str(), m_iPort, m_strUser.c_str(), m_strPass.c_str(), m_iProtocol, m_strLocalIP.c_str());
	else
		iResult = m_pUserAPI->Login(m_strHost.c_str(), m_iPort, m_strUser.c_str(), m_strPass.c_str(), m_iProtocol);
