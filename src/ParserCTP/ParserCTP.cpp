...
void ParserCTP::OnFrontConnected()
{
	if(m_parserSink)
	{
		m_parserSink->handleParserLog(LL_INFO, "[CTPParser]CTP行情服务已连接");
...
void ParserCTP::OnFrontDisconnected( int nReason )
{
	if(m_parserSink)
	{
		m_parserSink->handleParserLog(LL_ERROR, StrUtil::printf("[CTPParser]CTP行情服务连接已断开,原因: %d...", nReason).c_str());
...
void ParserCTP::OnRspSubMarketData( CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	...
	{
		//if(m_parserSink)
		//	m_parserSink->handleParserLog(LL_INFO, StrUtil::printf("[CTPParser]实时行情订阅成功,合约代码:%s", pSpecificInstrument->InstrumentID).c_str());
	}
	else
	{
		//if(m_parserSink)
		//	m_parserSink->handleParserLog(LL_INFO, StrUtil::printf("[CTPParser]实时行情订阅失败,合约代码:%s", pSpecificInstrument->InstrumentID).c_str());
	}
}
...
void ParserCTP::ReqUserLogin()
{
	...
	{
		if(m_parserSink)
			m_parserSink->handleParserLog(LL_ERROR, StrUtil::printf("[CTPParser]CTP 登录请求发送失败, 错误码:%d", iResult).c_str());
	}
}
...
void ParserCTP::SubscribeMarketData()
{
	...
	if(m_pUserAPI && nCount > 0)
	{
		...
		{
			if(m_parserSink)
				m_parserSink->handleParserLog(LL_ERROR, StrUtil::printf("[CTPParser]CTP 行情订阅请求发送失败, 错误码:%d", iResult).c_str());
		}
		else
		{
			if(m_parserSink)
				m_parserSink->handleParserLog(LL_INFO, StrUtil::printf("[CTPParser]一共订阅 %d 个合约行情", nCount).c_str());
		}
	}
	codeFilter.clear();
	delete[] subscribe;
